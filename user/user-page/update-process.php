...
if (isset($_POST['register'])) {
	...
	if (!preg_match($alpha_name_pattern, $first_name)) {
		...
		header("location:user.php?update_msg=$msg&color=danger&post_id=".$post_id."&blog_id=".$blog_id."");
	} else if ($first_name == "") {
	...
	if ($msg == "") {
		...
		if (isset($_FILES['upload_image']) && $_FILES['upload_image']['name'] != NULL) {
			...
			if ($result) {
				...
				header("location:../".$page."?update_msg=Account Update Success&color=success&post_id=".$post_id."&blog_id=".$blog_id."");
			}
		} else {
